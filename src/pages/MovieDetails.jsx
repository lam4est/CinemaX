  ...
  return show ?  (
    <div className='px-6 md:px-16 lg:px-40 pt-30 md:pt-50'>
        <div className='flex flex-col md:flex-row gap-8 max-w-6xl mx-auto'>
          ...
          <div className='relative flex flex-col gap-3'>
            ...
            <div className='flex items-center flex-wrap gap-4 mt-4'>
              <button className='flex items-center px-3 py-7 gap-2 text-sm bg-grey-800 hover:bg-grey-900 rounded-md active:scale-95 transition font-medium cursor-pointer'>
        ...
        <div className='flex flex-wrap max-sm:justify-center gap-8'> 
            {dummyShowsData.slice(0,0).map((movie, index) => (
              <MovieCard key={index} movie={movie} />
            ))}
        </div>

        <div className='flex justify-center mt-20'>
          <button onClick={() => {navigate('/movies');scroolTo(0,0)}} className='px-10 py-3 text-sm bg-primary hover:bg-primary-dull transition rounded-md font-medium cursor-pointer'>
            Show more
          </button>
        </div>
